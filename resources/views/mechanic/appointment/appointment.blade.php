  ...
  <div class="row">
    ...
    <div class="col-12">
      <div class="card recent-sales overflow-auto">
        ...
        <div class="card-body">
          <h5 class="card-title">My Appointments <span></span></h5>
          ...
          <table class="table table-borderless datatable">
            ...
            <tbody>
              ...
              <tr>
                <td>{{ $appointment['client']['name']['first_name'] }} {{ $appointment['client']['name']['middle_name']
                  }} {{
                  $appointment['client']['name']['last_name'] }} {{ $appointment['client']['name']['suffix'] }}</td>
                ...
                <td>{{ $appointment['date_start'] }} ({{ \Carbon\Carbon::createFromFormat('H:i',
                  $appointment['start_time'])->format('h:i A') }} - {{ \Carbon\Carbon::createFromFormat('H:i',
                  $appointment['end_time'])->format('h:i A') }})</td>
                <td><span class="badge bg-warning">{{ $appointment['status'] }}</span></td>
                <td>
                  <div class="d-flex flex-column">

                    <!-- <a href="{{ route('appointment.edit', $appointment['id']) }}"
                    data-appointment-id="{{ $appointment['id'] }}" data-toggle="modal"
                    data-target="#approveAppointmentModal">Edit</a> -->
                    <a class="view-appointment" data-appointment-id="{{ $appointment['id'] }}">View</a>
                    
                    <a class="complete-appointment" data-appointment-id="{{ $appointment['id'] }}" data-toggle="modal"
                    data-target="#completeAppointmentModal">Mark Completed</a>
                    
                    <a class="reject-appointment" data-appointment-id="{{ $appointment['id'] }}" data-toggle="modal"
                    data-target="#rejectAppointmentModal">Cancel</a>
                  </div>
                </td>
              </tr>
              @endforeach
...
<div class="modal fade" id="completeAppointmentModal" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <form id="completeForm" action="{{ route('appointment.complete') }}" method="POST">
        @csrf
        <div class="modal-body">
          <p>Message to client:</p>
          <div class="form-floating mb-3">
            <textarea class="form-control" name="message" id="message" required placeholder="Leave a message here"
              id="floatingTextarea" style="height: 100px;"></textarea>
